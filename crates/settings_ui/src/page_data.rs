    ...
    fn general_settings_section(_cx: &App) -> Vec<SettingsPageItem> {
        vec![
            ...
            SettingsPageItem::SettingItem(SettingItem {
                files: USER,
                title: "Language",
                description: "Interface language. Supported values: en, zh-CN.",
                field: Box::new(SettingField {
                    json_path: Some("locale"),
                    pick: |settings_content| settings_content.workspace.locale.as_ref().or(DEFAULT_EMPTY_STRING),
                    write: |settings_content, value, _| {
                        settings_content.workspace.locale = value;
                    },
                }),
                metadata: Some(Box::new(SettingsFieldMetadata {
                    placeholder: Some("en or zh-CN"),
                    ..Default::default()
                })),
            }),
            SettingsPageItem::SettingItem(SettingItem {
...
fn appearance_page() -> SettingsPage {
    ...
    fn background_section() -> [SettingsPageItem; 2] {
        [
            SettingsPageItem::SectionHeader("Background"),
            SettingsPageItem::SettingItem(SettingItem {
                files: USER,
                title: "Background Image",
                description: "Path to an image file to use as the editor background (experimental.theme_overrides).",
                field: Box::new(SettingField {
                    json_path: Some("experimental.theme_overrides.background.image_file"),
                    pick: |settings_content| {
                        settings_content
                            .theme
                            .experimental_theme_overrides
                            .as_ref()
                            .and_then(|o| o.background_image.as_ref())
                            .or(DEFAULT_EMPTY_STRING)
                    },
                    write: |settings_content, value, _| match value {
                        Some(v) => {
                            settings_content
                                .theme
                                .experimental_theme_overrides
                                .get_or_insert_with(Default::default)
                                .background_image = Some(v);
                        }
                        None => {
                            if let Some(ref mut overrides) =
                                settings_content.theme.experimental_theme_overrides
                            {
                                overrides.background_image = None;
                            }
                        }
                    },
                }),
                metadata: Some(Box::new(SettingsFieldMetadata {
                    placeholder: Some("/path/to/image.jpg"),
                    ..Default::default()
                })),
            }),
        ]
