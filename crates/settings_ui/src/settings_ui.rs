    ...
    fn render(
        ...
        match self {
            SettingsPageItem::SectionHeader(header) => {
                SettingsSectionHeader::new(SharedString::from(t(header, locale))).into_any_element()
            }
            SettingsPageItem::SettingItem(setting_item) => {
            ...
            v_flex()
                ...
                .child(
                    h_flex()
                        .w_full()
                        .gap_1()
                        .child(Label::new(SharedString::from(t(setting_item.title, WorkspaceSettings::get_global(cx).locale))))
                        .when_some(
                            if sub_field {
                                None
                            } else {
                ...
                .child(
                    Label::new(SharedString::from(t(setting_item.description, WorkspaceSettings::get_global(cx).locale)))
                        .size(LabelSize::Small)
                        .color(Color::Muted)
                        .render_code_spans(),
                ),
...
impl SettingsWindow {
    fn new(
        ...
        let mut last_locale = WorkspaceSettings::get_global(cx).locale;
        cx.observe_global_in::<SettingsStore>(window, move |this, window, cx| {
            ...
            let new_locale = WorkspaceSettings::get_global(cx).locale;
