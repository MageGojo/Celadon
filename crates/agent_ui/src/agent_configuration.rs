...
use workspace::{Workspace, WorkspaceSettings, create_and_open_local_file};
...
impl AgentConfiguration {
    ...
    fn render_provider_configuration_section(
        ...
        let popover_menu = PopoverMenu::new("add-provider-popover")
            .trigger(
                Button::new("add-provider", t("Add Provider", locale))
                    .style(ButtonStyle::Outlined)
                    .start_icon(
            ...
            .menu({
                ...
                move |window, cx| {
                    Some(ContextMenu::build(window, cx, |menu, _window, _cx| {
                        menu.header(t("Compatible APIs", locale)).entry("OpenAI", None, {
        ...
        v_flex()
            ...
            .child(self.render_section_title(
                t("LLM Providers", locale),
                t("Add at least one provider to use AI-powered features with Zed's native agent.", locale),
    ...
    fn render_context_servers_section(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let add_server_popover = PopoverMenu::new("add-server-popover")
            .trigger(
                Button::new("add-server", t("Add Server", locale))
                    .style(ButtonStyle::Outlined)
                    .start_icon(
                        Icon::new(IconName::Plus)
                            .size(IconSize::Small)
            ...
            .menu({
                move |window, cx| {
                    Some(ContextMenu::build(window, cx, |menu, _window, _cx| {
                        menu.entry(t("Add Custom Server", locale), None, {
                            |window, cx| {
                                window.dispatch_action(crate::AddContextServer.boxed_clone(), cx)
                            }
                        })
                        .entry(t("Install from Extensions", locale), None, {
                            |window, cx| {
        ...
        v_flex()
            ...
            .child(self.render_section_title(
                t("Model Context Protocol (MCP) Servers", locale),
                t("All MCP servers connected directly or via a Zed extension.", locale),
                        ...
                        if context_server_ids.is_empty() {
                            parent.child(
                                h_flex()
                                    ...
                                    .rounded_sm()
                                    .child(
                                        Label::new(t("No MCP servers added yet.", locale))
                                            .color(Color::Muted)
    ...
    fn render_agent_servers_section(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let add_agent_popover = PopoverMenu::new("add-agent-server-popover")
            .trigger(
                Button::new("add-agent", t("Add Agent", locale))
                    .style(ButtonStyle::Outlined)
                    .start_icon(
                        Icon::new(IconName::Plus)
                            .size(IconSize::Small)
            ...
            .menu({
                move |window, cx| {
                    Some(ContextMenu::build(window, cx, |menu, _window, _cx| {
                        menu.entry(t("Install from Registry", locale), None, {
                            |window, cx| {
                                window.dispatch_action(Box::new(zed_actions::AcpRegistry), cx)
                            }
                        })
                        .entry(t("Add Custom Agent", locale), None, {
                            move |window, cx| {
        ...
        v_flex()
            ...
            .child(
                v_flex()
                    .child(self.render_section_title(
                        t("External Agents", locale),
                        t("All agents connected through the Agent Client Protocol.", locale),
