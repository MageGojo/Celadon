...
use i18n::t;
use settings::{Settings as _, OpenAiCompatibleSettingsContent, update_settings_file};
...
impl AddLlmProviderInput {
    fn new(provider: LlmCompatibleProvider, window: &mut Window, cx: &mut App) -> Self {
        ...
        let provider_name =
            single_line_input(t("Provider Name", locale), provider.name(), None, 1, window, cx);
        let api_url = single_line_input(t("API URL", locale), provider.api_url(), None, 2, window, cx);
...
impl ModelInput {
    fn new(model_index: usize, window: &mut Window, cx: &mut App) -> Self {
        ...
        let max_completion_tokens = single_line_input(
            t("Max Completion Tokens", locale),
            "200000",
            Some("200000"),
            base_tab_index + 2,
            window,
    ...
    fn parse(&self, cx: &App) -> Result<AvailableModel, SharedString> {
        ...
        if name.is_empty() {
            return Err(SharedString::from(t("Model Name cannot be empty", locale)));
        }
        Ok(AvailableModel {
            name,
            display_name: None,
            max_completion_tokens: Some(
                self.max_completion_tokens
                    ...
                    .text(cx)
                    .parse::<u64>()
                    .map_err(|_| SharedString::from(t("Max Completion Tokens must be a number", locale)))?,
            ),
            max_output_tokens: Some(
                self.max_output_tokens
                    ...
                    .text(cx)
                    .parse::<u64>()
                    .map_err(|_| SharedString::from(t("Max Output Tokens must be a number", locale)))?,
            ),
            max_tokens: self
                .max_tokens
                .read(cx)
                .text(cx)
                .parse::<u64>()
                .map_err(|_| SharedString::from(t("Max Tokens must be a number", locale)))?,
...
fn save_provider_to_settings(
    ...
    if provider_name.is_empty() {
        return Task::ready(Err(t("Provider Name cannot be empty", locale).into()));
    }
        ...
        })
    {
        return Task::ready(Err(
            t("Provider Name is already taken by another provider", locale).into()
        ));
    }

    let api_url = input.api_url.read(cx).text(cx);
    if api_url.is_empty() {
        return Task::ready(Err(t("API URL cannot be empty", locale).into()));
    }

    let api_key = input.api_key.read(cx).text(cx);
    if api_key.is_empty() {
        return Task::ready(Err(t("API Key cannot be empty", locale).into()));
    }
        ...
        match model.parse(cx) {
            Ok(model) => {
                if !model_names.insert(model.name.clone()) {
                    return Task::ready(Err(t("Model Names must be unique", locale).into()));
                }
                models.push(model)
            }
    ...
    cx.spawn(async move |cx| {
        task.await
            .map_err(|_| SharedString::from(t("Failed to write API key to keychain", locale)))?;
        cx.update(|cx| {
    ...
    fn render_model_section(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    .justify_between()
                    .child(Label::new(t("Models", locale)).size(LabelSize::Small))
                    .child(
                        Button::new("add-model", t("Add Model", locale))
                            .start_icon(
    ...
    fn render_model(&self, ix: usize, cx: &mut Context<Self>) -> impl IntoElement + use<> {
        ...
        v_flex()
                ...
                v_flex()
                    ...
                    .child(
                        Checkbox::new(("supports-images", ix), model.capabilities.supports_images)
                            .label(t("Supports images", locale))
                            .on_click(cx.listener(move |this, checked, _window, cx| {
                    ...
                    .child(
                        ...
                        .label(t("Supports parallel_tool_calls", locale))
                    ...
                    .child(
                        ...
                        .label(t("Supports prompt_cache_key", locale))
                        .on_click(cx.listener(
                            move |this, checked, _window, cx| {
                    ...
                    .child(
                        ...
                        .label(t("Supports /chat/completions", locale))
                        .on_click(cx.listener(
                            move |this, checked, _window, cx| {
            ...
            .when(has_more_than_one_model, |this| {
                this.child(
                    Button::new(("remove-model", ix), t("Remove Model", locale))
                        .start_icon(
        ...
        v_flex()
            ...
            .child(
                Modal::new("configure-context-server", None)
                    .header(ModalHeader::new().headline(t("Add LLM Provider", locale)).description(
                        match self.provider {
                            LlmCompatibleProvider::OpenAi => {
                                t("This provider will use an OpenAI compatible API.", locale)
                            }
                        },
                    ))
                    ...
                    .footer(
                        ModalFooter::new().end_slot(
                            h_flex()
                                .gap_1()
                                .child(
                                    Button::new("cancel", t("Cancel", locale))
                                        .key_binding(
                                ...
                                .child(
                                    Button::new("save-server", t("Save Provider", locale))
                                        .key_binding(
                                            KeyBinding::for_action_in(
                                                &menu::Confirm,
                                                &focus_handle,
