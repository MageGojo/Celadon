...
use workspace::{
    ...
    ToggleWorkspaceSidebar, ToggleZoom, Workspace, WorkspaceId, WorkspaceSettings,
                ...
                move |window, cx| {
                    ...
                    Some(ContextMenu::build(window, cx, |mut menu, _window, _| {
                        ...
                        if can_regenerate_thread_title {
                            menu = menu.header(t("Current Thread", locale));

                            if let Some(conversation_view) = conversation_view.as_ref() {
                                menu = menu
                                    .entry(t("Regenerate Thread Title", locale), None, {
                                        let conversation_view = conversation_view.clone();
                                        move |_, cx| {
                                            Self::handle_regenerate_thread_title(
                                                conversation_view.clone(),
                        ...
                        if !showing_terminal {
                            menu = menu
                                .header("MCP Servers")
                                .action(
                                    t("View Server Extensions", locale),
                                ...
                                .action(t("Add Custom Server\u{2026}", locale), Box::new(AddContextServer))
                                .separator();

                            if !skills_enabled {
                                menu = menu.action(t("Rules", locale), Box::new(OpenRulesLibrary::default()));
                            }

                            menu = menu.action(t("Profiles", locale), Box::new(ManageProfiles::default()));
                        }

                        menu = menu
                            .action(t("Settings", locale), Box::new(OpenSettings))
                            .separator()
                            .action(t("Toggle Threads Sidebar", locale), Box::new(ToggleWorkspaceSidebar));

                        if has_auth_methods {
                            menu = menu.action(t("Reauthenticate", locale), Box::new(ReauthenticateAgent))
                        }
