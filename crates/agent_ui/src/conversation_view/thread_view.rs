    ...
    fn build_add_context_menu(
        ...
        ContextMenu::build(window, cx, move |menu, _window, _cx| {
            menu.key_context("AddContextMenu")
                ...
                .item(
                    ContextMenuEntry::new(t("Selection"))
    ...
    fn render_follow_toggle(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let tooltip_label = if following {
            if self.agent_id.as_ref() == agent::ZED_AGENT_ID.as_ref() {
                format!("{} {}", t("Stop Following the", locale), self.agent_id)
            } else {
                format!("{} {}", t("Stop Following", locale), self.agent_id)
            }
        } else {
            if self.agent_id.as_ref() == agent::ZED_AGENT_ID.as_ref() {
                format!("{} {}", t("Follow the", locale), self.agent_id)
            } else {
                format!("{} {}", t("Follow", locale), self.agent_id)
            }
        };
        let follow_meta = t("Track the agent's location as it reads and edits files.", locale);
