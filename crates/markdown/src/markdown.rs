...
pub struct MarkdownStyle {
    ...
    /// Bottom margin between paragraphs (px). Default 8.
    pub paragraph_spacing: f32,
    /// Paragraph line height multiplier. Default 1.3.
    pub paragraph_line_height: f32,
    /// Top margin before headings (rems). Default 1.
    pub heading_margin_top: f32,
    /// Override table header background. None = use theme.
    pub table_header_background: Option<Hsla>,
    /// Override table stripe row background. None = use theme.
    pub table_stripe_background: Option<Hsla>,
    /// Override table border color. None = use theme.
    pub table_border_color: Option<Hsla>,
}
    ...
    pub fn themed_with_overrides(
        ...
        MarkdownStyle {
            ...
            code_block: StyleRefinement {
                ...
                border_color: if is_preview { Some(gpui::rgb(0xe1e4e8u32).into()) } else { Some(colors.border_variant) },
                background: if is_preview { Some(gpui::rgb(0xf6f8fau32).into()) } else { Some(colors.editor_background.into()) },
                text: TextStyleRefinement {
                    ...
                    color: if is_preview { Some(gpui::rgb(0x24292eu32).into()) } else { None },
                    ..Default::default()
                },
                ..Default::default()
            },
            inline_code: TextStyleRefinement {
                ...
                font_size: Some(buffer_font_size.into()),
                font_weight: Some(buffer_font_weight),
                background_color: if is_preview { Some(gpui::Hsla::from(gpui::rgb(0xafb8c1u32)).opacity(0.2)) } else { Some(colors.editor_foreground.opacity(0.08)) },
                color: if is_preview { Some(gpui::rgb(0x24292eu32).into()) } else { None },
                ..Default::default()
            },
            link: TextStyleRefinement {
                background_color: if is_preview { None } else { Some(colors.editor_foreground.opacity(0.025)) },
                color: if is_preview { Some(gpui::rgb(0x0969dau32).into()) } else { Some(colors.text_accent) },
            ...
            heading_level_styles: if matches!(font, MarkdownFont::Agent) {
                Some(HeadingLevelStyles {
            ...
            } else if matches!(font, MarkdownFont::Preview) {
                Some(HeadingLevelStyles {
                    h1: Some(TextStyleRefinement {
            ...
            paragraph_spacing: if is_preview { 16. } else { 8. },
            paragraph_line_height: if is_preview { 1.7 } else { 1.3 },
            heading_margin_top: if is_preview { 1.5 } else { 1.0 },
            table_header_background: if is_preview { Some(gpui::rgb(0xf0f3f6u32).into()) } else { None },
            table_stripe_background: if is_preview { Some(gpui::rgb(0xfafbfcu32).into()) } else { None },
            table_border_color: if is_preview { Some(gpui::rgb(0xd0d7deu32).into()) } else { None },
            ..Default::default()
        }
    }
...
impl MarkdownElement {
    ...
    fn push_markdown_paragraph(
        ...
        let spacing = self.style.paragraph_spacing;
        let line_h = self.style.paragraph_line_height;
        let mut paragraph = div().when(!self.style.height_is_multiple_of_line_height, |el| {
            el.mb(gpui::px(spacing)).line_height(rems(line_h))
        });
    ...
    fn push_markdown_heading(
        ...
        let align = text_align_override.unwrap_or(self.style.base_text_style.text_align);
        let h_mt = self.style.heading_margin_top;
        let mut heading = div().mt(rems(h_mt)).mb_2();
                    ...
                    match tag {
                        ...
                        MarkdownTag::Table(alignments) => {
                            ...
                            builder.push_div(
                                div()
                                    ...
                                    .mb_2()
                                    .border(px(1.5))
                                    .border_color(self.style.table_border_color.unwrap_or_else(|| cx.theme().colors().border))
                                    .rounded_sm()
                                    .overflow_hidden(),
                                range,
                                markdown_end,
                        ...
                        MarkdownTag::TableCell => {
                            ...
                            let mut cell_div = div()
                                ...
                                .py_0p5()
                                .when(is_header, |this| {
                                    let bg = self.style.table_header_background.unwrap_or_else(|| cx.theme().colors().title_bar_background);
                                    this.bg(bg)
                                })
                                .when(!is_header && row_index % 2 == 1, |this| {
                                    let bg = self.style.table_stripe_background.unwrap_or_else(|| cx.theme().colors().panel_background);
                                    this.bg(bg)
                                });
