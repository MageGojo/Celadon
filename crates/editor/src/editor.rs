...
pub struct Editor {
    ...
    read_only: bool,
    http_response_key: Option<String>,
...
impl Editor {
    ...
    pub fn http_response_key(&self) -> Option<&str> {
        self.http_response_key.as_deref()
    }

    pub fn set_http_response_key(&mut self, key: String, _cx: &mut Context<Self>) {
        self.http_response_key = Some(key);
    }
    ...
    fn create_style(&self, cx: &App) -> EditorStyle {
        ...
        let background = match self.mode {
            ...
            EditorMode::Full { .. } => {
                ...
                if cx.theme().styles.background_image.is_some() {
                    bg.opacity(bg.a * 0.85)
                } else {
                    bg
                }
            }
