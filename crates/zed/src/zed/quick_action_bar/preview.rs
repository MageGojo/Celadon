...
use html_preview::{
    OpenPreview as HtmlOpenPreview, OpenPreviewToTheSide as HtmlOpenPreviewToTheSide,
    html_preview_view::HtmlPreviewView,
};
    ...
    pub fn render_preview_button(
        ...
        if let Some(workspace) = self.workspace.upgrade() {
            workspace.update(cx, |workspace, cx| {
                if SvgPreviewView::resolve_active_item_as_svg_buffer(workspace, cx).is_some()
                {
                ...
                } else if HtmlPreviewView::resolve_active_item_as_html_buffer(workspace, cx)
                    .is_some()
                {
                    preview_type = Some(PreviewType::Html);
                }
            });
        }
        ...
        let (button_id, tooltip_text, open_action, open_to_side_action, open_action_for_tooltip) =
            match preview_type {
                ...
                PreviewType::Html => (
                    "toggle-html-preview",
                    "Preview HTML",
                    Box::new(HtmlOpenPreview) as Box<dyn gpui::Action>,
                    Box::new(HtmlOpenPreviewToTheSide) as Box<dyn gpui::Action>,
                    &html_preview::OpenPreview as &dyn gpui::Action,
                ),
            };
