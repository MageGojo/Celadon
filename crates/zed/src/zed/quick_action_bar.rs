...
use editor::mouse_context_menu::parse_http_request;
use workspace::{OpenHttpResponse, RunHttpRequest};
...
impl QuickActionBar {
    ...
    fn render_http_run_button(&mut self, cx: &mut Context<Self>) -> Option<impl IntoElement> {
        use editor::ToPoint as _;

        let editor = self.active_editor()?;

        let (is_http, content, cursor_row) = editor.update(cx, |editor, cx| {
            let singleton = editor.buffer().read(cx).as_singleton().map(|e| e.clone());
            let is_http = singleton
                .as_ref()
                .and_then(|buf| {
                    buf.read(cx)
                        .file()
                        .and_then(|f| f.path().extension().map(|e| matches!(e, "http" | "rest")))
                })
                .unwrap_or(false);
        ...
        let action = parse_http_request(&content, cursor_row);
        let is_running = self.http_running;
        let icon = if is_running { IconName::ArrowCircle } else { IconName::PlayFilled };

        let button = IconButton::new("run-http-request", icon)
            .icon_size(IconSize::Small)
            .style(ButtonStyle::Subtle)
            .disabled(action.is_none() || is_running)
            .tooltip(if is_running {
                Tooltip::text("Running…")
            } else {
                Tooltip::text("Run HTTP Request")
            })
            .when_some(action, |btn, act| {
                btn.on_click(cx.listener(move |this, _, window, cx| {
                    this.http_running = true;
                    cx.notify();
                    window.dispatch_action(Box::new(act.clone()), cx);
                }))
            });
...
impl Render for QuickActionBar {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
                ...
                .menu(move |window, cx| {
                    ...
                    let menu = ContextMenu::build(window, cx, move |menu, _, _| {
                        menu.context(focus.clone())
                            .action(t("Select All", locale), Box::new(SelectAll))
                            .action(
                            ...
                            .action(t("Expand Selection", locale), Box::new(SelectLargerSyntaxNode))
                            .action(t("Shrink Selection", locale), Box::new(SelectSmallerSyntaxNode))
                            .action(
                            ...
                            .when(!disable_ai, |this| {
                                this.separator().action_disabled_when(
                                    !has_selection,
                                    t("Add to Agent Thread", locale),
                                    Box::new(AddSelectionToThread),
                                )
                            })
                            .separator()
                            .action(t("Go to Symbol", locale), Box::new(ToggleOutline))
                            .action(t("Go to Line/Column", locale), Box::new(ToggleGoToLine))
                            .separator()
                            .action(t("Next Problem", locale), Box::new(GoToDiagnostic::default()))
                            .action(
                                t("Previous Problem", locale),
                                Box::new(GoToPreviousDiagnostic::default()),
                            )
                            .separator()
                            .action_disabled_when(!has_diff_hunks, t("Next Hunk", locale), Box::new(GoToHunk))
                            .action_disabled_when(
                                !has_diff_hunks,
                                t("Previous Hunk", locale),
                                Box::new(GoToPreviousHunk),
                            )
                            .separator()
                            .action(t("Move Line Up", locale), Box::new(MoveLineUp))
                            .action(t("Move Line Down", locale), Box::new(MoveLineDown))
                            .action(t("Duplicate Selection", locale), Box::new(DuplicateLineDown))
                    });
                    Some(menu)
                })
        });
        ...
        let editor_settings_dropdown = {
            ...
            PopoverMenu::new("editor-settings")
                .trigger_with_tooltip(
                    ...
                    Tooltip::text(t("Editor Controls", locale)),
                )
                .anchor(Anchor::TopRight)
                ...
                .menu(move |window, cx| {
                    let menu = ContextMenu::build(window, cx, {
                        ...
                        |mut menu, _, _| {
                            ...
                            if supports_minimap {
                                menu = menu.toggleable_entry(t("Minimap", locale), minimap_enabled, IconPosition::Start, Some(editor::actions::ToggleMinimap.boxed_clone()), {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
                                            .update(cx, |editor, cx| {
                            ...
                            if has_edit_prediction_provider {
                                let mut edit_prediction_entry = ContextMenuEntry::new(t("Edit Predictions", locale))
                            ...
                            if is_full {
                                ...
                                if supports_inline_diagnostics {
                                    let mut inline_diagnostics_item = ContextMenuEntry::new(t("Inline Diagnostics", locale))
        ...
        h_flex()
            ...
            .on_action(cx.listener(|this, _: &workspace::OpenHttpResponse, _, cx| {
                this.http_running = false;
                cx.notify();
            }))
            .children(self.render_repl_menu(cx))
            .children(self.render_preview_button(self.workspace.clone(), cx))
            .children(self.render_http_run_button(cx))
