...
use workspace::{
    ...
    MoveItemToPaneInDirection, MovePaneDown, MovePaneLeft, MovePaneRight, MovePaneUp, OpenHttpResponse,
...
pub fn init(cx: &mut App) {
    cx.observe_new(
        |workspace: &mut Workspace, _window, _: &mut Context<Workspace>| {
            ...
            workspace.register_action(Workspace::run_file_path_in_terminal);
            workspace.register_action(Workspace::run_http_request);
            workspace.register_action(TerminalPanel::open_http_response);
...
impl TerminalPanel {
    ...
    pub fn open_http_response(
        ...
        // Try to find and reuse an existing tab for this request
        let existing_editor = workspace
            .active_pane()
            .read(cx)
            .items()
            .filter_map(|item| item.act_as::<editor::Editor>(cx))
            .find(|editor| {
                editor.read(cx).http_response_key().as_deref() == Some(&request_key)
            });
        ...
        if let Some(editor) = existing_editor {
            ...
            editor.update(cx, |ed, cx| {
                ...
                if let Some(buffer) = ed.buffer().read(cx).as_singleton() {
                    buffer.update(cx, |buf, cx| {
                        let len = buf.len();
                        buf.edit([(0..len, text)], None, cx);
                    });
                }
                ed.set_read_only(true);
                ed.set_scroll_position(gpui::Point::new(0.0.into(), 0.0.into()), window, cx);
            });
        ...
        cx.spawn_in(window, async move |workspace, cx| {
            let language = languages.language_for_name("http").await.ok();
            workspace.update_in(cx, |workspace, window, cx| {
                let buffer = project.update(cx, |project, cx| {
                    project.create_local_buffer(&text, language, true, cx)
                });
                let editor = cx.new(|cx| {
                    editor::Editor::for_buffer(buffer, Some(project.clone()), window, cx)
                });
                editor.update(cx, |ed, cx| {
                    ed.set_http_response_key(request_key, cx);
                    ed.set_read_only(true);
                });
