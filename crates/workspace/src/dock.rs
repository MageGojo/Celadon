        ...
        let mut buttons: Vec<_> = dock
            ...
            .filter_map(|(i, entry)| {
                ...
                let (action, tooltip) = if is_active_button {
                    ...
                    let tooltip: SharedString = match (locale, dock.position) {
                        (Locale::ZhCn, DockPosition::Left) => "关闭左侧面板".into(),
                        (Locale::ZhCn, DockPosition::Right) => "关闭右侧面板".into(),
                        (Locale::ZhCn, DockPosition::Bottom) => "关闭底部面板".into(),
                        _ => format!("Close {} Dock", dock.position.label()).into(),
                    };
                    (action, tooltip)
                } else {
                    let action = entry.panel.toggle_action(window, cx);
                    let translated = t(icon_tooltip, locale);
                    (action, SharedString::from(translated))
                };
                            ...
                            ContextMenu::build(window, cx, |mut menu, _, cx| {
                                ...
                                for position in POSITIONS {
                                    if panel.position_is_valid(position, cx) {
                                        ...
                                        let panel = panel.clone();
                                        let dock_label: SharedString = match (locale, position) {
                                            (Locale::ZhCn, DockPosition::Left) => "停靠左侧".into(),
                                            (Locale::ZhCn, DockPosition::Right) => "停靠右侧".into(),
                                            (Locale::ZhCn, DockPosition::Bottom) => "停靠底部".into(),
                                            _ => format!("Dock {}", position.label()).into(),
                                        };
