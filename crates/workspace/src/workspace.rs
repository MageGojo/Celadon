...
actions!(
    ...
    [
        ...
        OpenInTerminal,
        /// Runs the selected file in a terminal using the configured run_file_command.
        RunFileInTerminal,
...
/// Runs the specified file path in a terminal using the configured run_file_command.
#[derive(Debug, Clone, Deserialize, PartialEq, JsonSchema, Action)]
#[action(namespace = workspace)]
#[serde(deny_unknown_fields)]
pub struct RunFilePathInTerminal {
    pub path: PathBuf,
}

/// Opens an HTTP response buffer (dispatched after RunHttpRequest completes).
#[derive(Debug, Clone, Deserialize, PartialEq, JsonSchema, Action)]
#[action(namespace = workspace)]
#[serde(deny_unknown_fields)]
pub struct OpenHttpResponse {
    pub text: String,
    pub is_json: bool,
    pub request_key: String,
}

/// Runs an HTTP request from a .http file natively via Zed's HTTP client.
#[derive(Debug, Clone, Deserialize, PartialEq, JsonSchema, Action)]
#[action(namespace = workspace)]
#[serde(deny_unknown_fields)]
pub struct RunHttpRequest {
    pub method: String,
    pub url: String,
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .when_some(
                                theme.styles.background_image.clone(),
                                |this, image_path| {
                                    this.child(
                                        gpui::img(std::path::PathBuf::from(image_path.as_ref()))
                                            .absolute()
                                            .inset_0()
                                            .size_full()
