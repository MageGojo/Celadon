...
impl Workspace {
    ...
    pub fn run_file_path_in_terminal(
        self: &mut Workspace,
        action: &crate::RunFilePathInTerminal,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
        ...
        let run_cmd = WorkspaceSettings::get_global(cx).run_file_command.clone();
        let command = format!("{} {}", run_cmd, path.to_string_lossy());
        let label = command.clone();
        let spawn_task = SpawnInTerminal {
            id: TaskId(format!("run-file:{}", path.to_string_lossy())),
            full_label: label.clone(),
            label: label.clone(),
            command: Some(command),
            args: Vec::new(),
            command_label: label,
            cwd: path.parent().map(|p| p.to_path_buf()),
            env: HashMap::default(),
    ...
    pub fn run_http_request(
        self: &mut Workspace,
        action: &crate::RunHttpRequest,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
        let request_key = format!("{} {}", action.method, action.url);
        let task = Self::fetch_http_request(self, action, cx);
        cx.spawn_in(window, async move |workspace, cx| {
            let (response_text, is_json) = match task.await {
                Ok(data) => data,
                Err(err) => (format!("Error: {}\n", err), false),
            };
            workspace.update_in(cx, |_workspace, window, cx| {
                window.dispatch_action(
                    Box::new(crate::OpenHttpResponse {
                        text: response_text,
                        is_json,
                        request_key,
                    }),
    ...
    pub fn fetch_http_request(
        workspace: &Workspace,
        action: &crate::RunHttpRequest,
        cx: &mut Context<Workspace>,
    ) -> Task<anyhow::Result<(String, bool)>> {
        use http_client::{AsyncBody, Method, RedirectPolicy, Request, HttpRequestExt};

        let method: Method = action.method.parse().unwrap_or(Method::GET);
        ...
        cx.background_spawn(async move {
            ...
            let version = match response.version() {
                http_client::http::Version::HTTP_09 => "HTTP/0.9",
                http_client::http::Version::HTTP_10 => "HTTP/1.0",
                http_client::http::Version::HTTP_11 => "HTTP/1.1",
                http_client::http::Version::HTTP_2 => "HTTP/2",
                http_client::http::Version::HTTP_3 => "HTTP/3",
                _ => "HTTP",
            };
            ...
            response_text.push_str(&body_display);

            anyhow::Ok((response_text, is_json))
        })
    }
