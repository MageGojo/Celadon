use crate::{
    ...
    OpenTerminal, OpenVisible, RunFileInTerminal, RunFilePathInTerminal, SplitDirection,
    ToggleFileFinder, ToggleProjectSymbols, ToggleZoom,
                    ...
                    if let Some(pane) = pane.upgrade() {
                        menu = menu
                            ...
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(t("Close Others", locale))
                            ...
                            .extend(has_multibuffer_items.then(|| {
                                ContextMenuItem::Entry(
                                    ContextMenuEntry::new(t("Close Multibuffers", locale))
                            ...
                            .separator()
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(t("Close Left", locale))
                            ...
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(t("Close Right", locale))
                            ...
                            .separator()
                            .item(ContextMenuItem::Entry(
                                ContextMenuEntry::new(t("Close Clean", locale))
                        ...
                        if capability != Capability::ReadOnly {
                            let read_only_label = if capability.editable() {
                                t("Make File Read-Only", locale)
                            } else {
                                t("Make File Editable", locale)
                            };
                        ...
                        if let Some(entry) = single_entry_to_resolve {
                            ...
                            menu = menu
                                ...
                                .when(is_local, |menu| {
                                    menu.when_some(reveal_path, |menu, reveal_path| {
                                        menu.separator().entry(
                                            t(ui::utils::reveal_in_file_manager_label(is_remote), locale),
                                            Some(Box::new(
                                ...
                                .when(visible_in_project_panel, |menu| {
                                    menu.entry(
                                        t("Reveal In Project Panel", locale),
                                ...
                                .when_some(
                                    ...
                                    |menu, file_abs_path| {
                                        menu.entry(
                                            t("Run File", locale),
                                            Some(Box::new(RunFileInTerminal)),
                                            window.handler_for(&pane, move |_, window, cx| {
                                                window.dispatch_action(
                                                    RunFilePathInTerminal {
                                                        path: file_abs_path.clone(),
                                                    }
                                                    .boxed_clone(),
                                                    cx,
                                                );
        ...
        .child(
            PopoverMenu::new("pane-tab-bar-popover-menu")
                .trigger_with_tooltip(
                    ...
                    Tooltip::text(t("New...", locale)),
                )
                .anchor(Anchor::TopRight)
                .with_handle(pane.new_item_context_menu_handle.clone())
                .menu(move |window, cx| {
                    Some(ContextMenu::build(window, cx, |menu, _, _| {
                        menu.action(t("New File", locale), NewFile.boxed_clone())
                            .action(t("Open File", locale), ToggleFileFinder::default().boxed_clone())
                            .separator()
                            .action(t("Search Project", locale), DeploySearch::default().boxed_clone())
                            .action(t("Search Symbols", locale), ToggleProjectSymbols.boxed_clone())
                            .separator()
                            .action(t("New Terminal", locale), NewTerminal::default().boxed_clone())
                            .action(
                                t("New Center Terminal", locale),
        ...
        .child(
            PopoverMenu::new("pane-tab-bar-split")
                .trigger_with_tooltip(
                    ...
                    Tooltip::text(t("Split Pane", locale)),
                )
                .anchor(Anchor::TopRight)
                ...
                .menu(move |window, cx| {
                    ContextMenu::build(window, cx, |menu, _, _| {
                        ...
                        if can_split_move {
                            menu.action(t("Split Right", locale), SplitRight { mode }.boxed_clone())
                                .action(t("Split Left", locale), SplitLeft { mode }.boxed_clone())
                                .action(t("Split Up", locale), SplitUp { mode }.boxed_clone())
                                .action(t("Split Down", locale), SplitDown { mode }.boxed_clone())
                        } else {
                            menu.action(t("Split Right", locale), SplitRight::default().boxed_clone())
                                .action(t("Split Left", locale), SplitLeft::default().boxed_clone())
                                .action(t("Split Up", locale), SplitUp::default().boxed_clone())
                                .action(t("Split Down", locale), SplitDown::default().boxed_clone())
                        }
                    })
                    .into()
                }),
        ...
        .child({
            ...
            IconButton::new("toggle_zoom", IconName::Maximize)
                ...
                .tooltip(move |_window, cx| {
                    Tooltip::for_action(
                        if zoomed { t("Zoom Out", locale) } else { t("Zoom In", locale) },
                        &ToggleZoom,
                        cx,
                    )
                })
