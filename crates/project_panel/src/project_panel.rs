...
use i18n::t;
use workspace::{
    DraggedSelection, OpenInTerminal, RunFileInTerminal, RunFilePathInTerminal, OpenMode, OpenOptions, OpenVisible, PreviewTabsSettings,
...
impl ProjectPanel {
    ...
    fn deploy_context_menu(
        ...
        if let Some((worktree, entry)) = self.selected_sub_entry(cx) {
            ...
            let reveal_label = t(ui::utils::reveal_in_file_manager_label(is_remote), locale);
            let context_menu = ContextMenu::build(window, cx, |menu, _, cx| {
                menu.context(self.focus_handle.clone()).map(|menu| {
                    if is_read_only {
                        menu.when(is_dir, |menu| {
                            menu.action(t("Search Inside", locale), Box::new(NewSearchInDirectory))
                        })
                    } else {
                        menu.action(t("New File", locale), Box::new(NewFile))
                            .action(t("New Folder", locale), Box::new(NewDirectory))
                            .separator()
                            .when(is_local, |menu| {
                                menu.action(reveal_label, Box::new(RevealInFileManager))
                            })
                            .when(is_local, |menu| {
                                menu.action(t("Open in Default App", locale), Box::new(OpenWithSystem))
                            })
                            .action(t("Open in Terminal", locale), Box::new(OpenInTerminal))
                            .when(!is_dir, |menu| {
                                menu.action(t("Run File", locale), Box::new(RunFileInTerminal))
                            })
                            .when(is_dir, |menu| {
                                menu.separator()
                                    .action(t("Find in Folder…", locale), Box::new(NewSearchInDirectory))
                            })
                            .when(is_unfoldable, |menu| {
                                menu.action(t("Unfold Directory", locale), Box::new(UnfoldDirectory))
                            })
                            .when(is_foldable, |menu| {
                                menu.action(t("Fold Directory", locale), Box::new(FoldDirectory))
                            })
                            .when(should_show_compare, |menu| {
                                menu.separator()
                                    .action(t("Compare Marked Files", locale), Box::new(CompareMarkedFiles))
                            })
                            .separator()
                            .action(t("Cut", locale), Box::new(Cut))
                            .action(t("Copy", locale), Box::new(Copy))
                            .action(t("Duplicate", locale), Box::new(Duplicate))
                            // TODO: Paste should always be visible, cbut disabled when clipboard is empty
                            .action_disabled_when(!has_pasteable_content, t("Paste", locale), Box::new(Paste))
                            ...
                            .when(is_remote, |menu| {
                                menu.separator()
                                    .action(t("Download...", locale), Box::new(DownloadFromRemote))
                            })
                            .separator()
                            .action(t("Copy Path", locale), Box::new(zed_actions::workspace::CopyPath))
                            .action(
                                t("Copy Relative Path", locale),
                            ...
                            .when(has_git_repo, |menu| {
                                menu.separator()
                                    .when(!is_dir && self.has_git_changes(entry_id), |menu| {
                                        menu.action(
                                            t("Restore File", locale),
                                            Box::new(git::RestoreFile { skip_prompt: false }),
                                        )
                                    })
                                    .action(t("Add to .gitignore", locale), Box::new(git::AddToGitignore))
                                    .when(has_history, |menu| {
                                        menu.action(t("View History", locale), Box::new(git::FileHistory))
                                    })
                            })
                            .when(!should_hide_rename, |menu| {
                                menu.separator().action(t("Rename", locale), Box::new(Rename))
                            })
                            .when(!is_root && !is_remote, |menu| {
                                menu.action(t("Trash", locale), Box::new(Trash { skip_prompt: false }))
                            })
                            .when(!is_root, |menu| {
                                menu.action(t("Delete", locale), Box::new(Delete { skip_prompt: false }))
                            })
                            .when(!is_collab && is_root, |menu| {
                                menu.separator()
                                    .action(
                                        t("Add Folders to Project…", locale),
                                        Box::new(workspace::AddFolderToProject),
                                    )
                                    .action(t("Remove from Project", locale), Box::new(RemoveFromProject))
                            })
                            .when(is_dir && !is_root, |menu| {
                                menu.separator().action(
                                    t("Collapse All", locale),
                            ...
                            .when(is_dir && is_root, |menu| {
                                ...
                                menu.separator().item(
                                    ContextMenuEntry::new(t("Collapse All", locale)).handler(
                                        move |window, cx| {
    ...
    fn run_file_in_terminal(
        ...
        window.dispatch_action(
            RunFilePathInTerminal { path: abs_path }.boxed_clone(),
            cx,
        );
    }
                ...
                .when(
                    ...
                    |el| {
                        el.on_action(cx.listener(Self::reveal_in_finder))
                            ...
                            .on_action(cx.listener(Self::run_file_in_terminal))
                    },
                )
