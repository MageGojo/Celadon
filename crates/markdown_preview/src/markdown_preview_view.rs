...
fn resolve_data_url_image(dest_url: &str) -> Option<ImageSource> {
    let rest = dest_url.strip_prefix("data:")?;
    let comma_pos = rest.find(',')?;
    let meta = &rest[..comma_pos];
    let data = &rest[comma_pos + 1..];

    let format = if meta.starts_with("image/png") {
        gpui::ImageFormat::Png
    } else if meta.starts_with("image/jpeg") || meta.starts_with("image/jpg") {
        gpui::ImageFormat::Jpeg
    } else if meta.starts_with("image/gif") {
        gpui::ImageFormat::Gif
    } else if meta.starts_with("image/webp") {
        gpui::ImageFormat::Webp
    } else if meta.starts_with("image/svg") {
        gpui::ImageFormat::Svg
    } else if meta.starts_with("image/bmp") {
...
fn resolve_preview_image(
    ...
    if dest_url.starts_with("data:") {
        return resolve_data_url_image(dest_url);
    }
